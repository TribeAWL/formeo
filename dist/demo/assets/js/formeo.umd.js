...
(function(global2, factory) {
      ...
      (src) => new Promise((resolve, reject) => {
        ...
        const styleLink = dom.create({
          ...
          action: {
            ...
            error: (e2) => reject(new Error(`${e2.target.src} failed to load.`))
          }
        });
  ...
  class Component extends Data {
          ...
          (controlData) => {
            if (this.name === "section") {
              return (childData, childIndex) => {
                ...
                if (controlType === "row") {
                  const Rows2 = components.rows;
                  const row = Rows2.add(uuid(), childData);
                  const childWrap = this.dom.querySelector(".children");
                  ...
                  if (controlType === "field" || !controlId.startsWith("layout-")) {
                    const Fields2 = components.fields;
                    const field2 = Fields2.add(uuid(), childData);
                    const childWrap = this.dom.querySelector(".children");
                    if (childIndex >= childWrap.children.length) {
      ...
      const onAddConditions = {
        controls: async () => {
          ...
          if (controlType === "section") {
            ...
            const { default: Sections2 } = await Promise.resolve().then(() => index$9);
    ...
    clone = (parent = this.parent) => {
      ...
      if (this.name === "field" && targetParent.name === "section") {
        const Fields2 = components.fields;
        const newField = Fields2.add(uuid(), this.cloneData());
  ...
  const DEFAULT_DATA$4 = () => Object.freeze({
    config: {
      width: "100%"
    },
    children: [],
  ...
  class Column extends Component {
    ...
    constructor(columnData) {
      super("column", { ...DEFAULT_DATA$4(), ...columnData });
  ...
  class Row extends Component {
     ...
     */
    constructor(rowData) {
      super("row", { ...DEFAULT_DATA$3(), ...rowData });
  ...
  class Section extends Component {
    ...
    async isFormField(item) {
      ...
      const { default: Controls2 } = await Promise.resolve().then(() => index$8);
      const controlData = Controls2.get(item.id);
      if (!controlData) {
        return false;
      }
  ...
  let Controls$1 = class Controls {
    ...
    getFieldOptions = (excludeFieldId) => {
      ...
      Promise.resolve().then(() => index$7).then(({ default: Fields2 }) => {
        const fieldsData = Fields2.getData() || {};
    ...
    refreshFieldOptions = async () => {
      const { default: Fields2 } = await Promise.resolve().then(() => index$7);
      const fieldsData = Fields2.getData() || {};
  ...
  function createRadioGroup(field2) {
    ...
    if (field2.options && Array.isArray(field2.options)) {
      field2.options.forEach((option, index2) => {
